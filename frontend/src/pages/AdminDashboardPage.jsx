...
const AdminDashboardPage = () => {
  ...
  if (!user) {
    return <div>Please log in as an admin/counselor.</div>;
  }

  if (user.role !== "counselor" && user.role !== "admin") {
    return <div>Access denied. This page is for counselors and admins only.</div>;
  }
              ...
              <div className="requests-list">
                {requests.map(request => (
                  <div key={request._id} className="request-item admin-request">
                    ...
                    <p><strong>Student:</strong> {request.student.name} ({request.student.email})</p>
                    ...
                    <div className="request-response">
                      ...
                      <div className="status-buttons">
                        <button 
                          ...
                          onClick={() => handleStatusChange(request._id, "in_progress", document.getElementById(`note-${request._id}`).value)}
                        >
                          In Progress
                        </button>
                        <button 
                          className="btn btn-small btn-success"
                          onClick={() => handleStatusChange(request._id, "closed", document.getElementById(`note-${request._id}`).value)}
                        >
                          Close
                        </button>
                      </div>
              ...
              <div className="resources-list">
                {resources.map(resource => (
                  <div key={resource._id} className="resource-item">
                    ...
                    {resource.link && (
                      <p><a href={resource.link} target="_blank" rel="noopener noreferrer">View Resource</a></p>
                    )}
